# 🚀 Dialogs RAG - Анализ диалогов с помощью ИИ
...
- 🌍 **Расширенная аналитика** - регион, сегмент, тональность, тип клиента
- 📋 **Автоматические отчеты** - Markdown и Excel с детальной статистикой
- ⚙️ **Гибкая конфигурация** - легко добавлять новые поля анализа
...
```bash
# Просто запустите скрипт - он сделает всё автоматически
./start_analysis.sh
```
...
# Или полный анализ в одном скрипте
...
- **report.md** - Markdown отчет с аналитикой
- **report.xlsx** - Excel отчет с детальными данными

### Расширенные поля анализа:
- **region** - регион/город клиента
- **segment** - сегмент клиента (PRO/M/S, VIP, корпоративный)
...
- **return_issue** - проблемы с возвратом
...
├── field_config.py        # Конфигурация полей анализа
...
├── aggregate.py           # Генерация отчетов
├── start_analysis.sh      # Скрипт быстрого запуска
├── batch_results.jsonl    # Результаты анализа (JSONL)
├── batch_results.csv      # Результаты анализа (CSV)
