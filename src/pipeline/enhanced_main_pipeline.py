"""Enhanced main pipeline with quality improvements"""
...
class EnhancedMainPipeline(BasePipeline):
    """Enhanced main pipeline with improved entity extraction quality"""
    
    def __init__(self, config_manager: ConfigManager, use_enhanced_extractor: bool = True):
        """Initialize enhanced main pipeline
        
        Args:
            config_manager: Configuration manager instance
            use_enhanced_extractor: Whether to use enhanced entity extractor
        """
        super().__init__(config_manager, "enhanced_main_pipeline")
        
        # Initialize pipeline stages
        self.dialog_processor = DialogProcessor(config_manager)
        
        # Choose entity extractor based on configuration
        if use_enhanced_extractor:
            self.entity_extractor = EnhancedEntityExtractor(config_manager)
            self.extractor_type = "enhanced"
        else:
            self.entity_extractor = EntityExtractor(config_manager)
            self.extractor_type = "standard"
    ...
    def process(self, data: Any, **kwargs) -> Dict[str, Any]:
        ...
        """
        self.log_stage_start(f"Using {self.extractor_type} entity extractor")
        ...
        try:
            ...
            for stage_name, stage in self.stages:
                try:
                    ...
                    if stage_name == "dialog_processing":
                        ...
                        if self.extractor_type == "enhanced":
                        ...
                        results['stages'][stage_name] = {
                            ...
                            'end_time': datetime.now().isoformat(),
                            'data_shape': current_data.shape if hasattr(current_data, 'shape') else len(current_data),
                            'flattened_entities_count': len(results.get('flattened_entities', [])),
                            'extractor_type': self.extractor_type
                        }
            ...
            self.log_stage_end(f"Enhanced pipeline completed successfully with {self.extractor_type} extractor")
            return results
            
        except Exception as e:
            self.log_error(e, "Enhanced pipeline failed")
            results['pipeline_end_time'] = datetime.now().isoformat()
    ...
    def _save_stage_result(self, stage_name: str, data: Any) -> None:
        ...
        try:
            ...
            filename = f"enhanced_{stage_name}_{timestamp}.pkl"
            ...
            if isinstance(data, pd.DataFrame):
                ...
                self.logger.info(f"Saved enhanced {stage_name} result to {filepath}")
            
        except Exception as e:
            self.logger.warning(f"Failed to save enhanced {stage_name} result: {e}")
    ...
    def _generate_final_results(self, data: pd.DataFrame, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            final_results = {
                ...
                'extractor_type': self.extractor_type,
                'quality_improvements': self._assess_quality_improvements(results)
            }
    ...
    def _assess_quality_improvements(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Assess quality improvements from enhanced extraction
        ...
        try:
            quality_metrics = {
                ...
                'context_included': 0,
                'business_relevance': 'high' if self.extractor_type == 'enhanced' else 'medium',
                'actionability': 'high' if self.extractor_type == 'enhanced' else 'medium'
            }
            
            # Analyze flattened entities for quality metrics
            if 'flattened_entities' in results:
                flattened_entities = results['flattened_entities']
                
                # Count structured entities (with entity_data field)
                structured_count = sum(1 for entity in flattened_entities 
                                     if isinstance(entity.get('entity_data'), dict))
                
                quality_metrics['structured_entities'] = structured_count
                quality_metrics['context_included'] = structured_count  # Enhanced extractor includes context
                
                # Calculate quality score
                total_entities = len(flattened_entities)
                if total_entities > 0:
                    quality_metrics['quality_score'] = (structured_count / total_entities) * 100
                else:
            ...
            self.log_error(e, "Failed to assess quality improvements")
            return {}
    ...
    def _save_final_results(self, results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(f"Saved enhanced final results to {filepath}")
            
        except Exception as e:
            self.log_error(e, "Failed to save enhanced final results")
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        """
        return {
            'pipeline_name': 'enhanced_main_pipeline',
            'pipeline_type': 'enhanced',
            'extractor_type': self.extractor_type,
